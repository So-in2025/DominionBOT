...
export async function connectToWhatsApp(userId: string, phoneNumber?: string) {
  // 1. Limpieza previa y bloqueo de reintentos rápidos
  await disconnectWhatsApp(userId);
  
  // AUMENTADO: Delay de seguridad de 2s a 5s para evitar Rate Limiting (Error 428)
  ...
  console.log(`[WA] Iniciando socket STANDARD (Ubuntu/Chrome) para ${userId}...`);
  ...
  try {
      const sock = makeWASocket({
        ...
        logger,
        // CORRECCIÓN CRÍTICA: Firma estándar de Ubuntu. 
        // "Dominion OS" causaba Error 405/428. WhatsApp espera un OS real.
        ...
        emitOwnEvents: false,
        retryRequestDelayMs: 2000, // Retraso entre peticiones internas
      });

      sessions.set(userId, sock);
      sock.ev.on('creds.update', saveCreds);

      // Lógica de Pairing Code (Teléfono)
      if (phoneNumber && !sock.authState.creds.registered) {
          setTimeout(async () => {
              try {
                  ...
                  // No desconectar inmediatamente, dejar que el usuario reintente o el socket muera solo
                  sseService.sendEvent(userId, 'status_update', { status: ConnectionStatus.DISCONNECTED });
              }
          }, 5000); // Aumentado a 5s para dar tiempo al socket a establecerse
      }
      ...
      sock.ev.on('connection.update', async (update) => {
          ...
          if (qr && !phoneNumber) {
            console.log(`[WA] QR Generado Exitosamente para ${userId}`);
          ...
          if (connection === 'close') {
            ...
            if (shouldReconnect) {
                ...
                // Si el error es 428 (Precondition Required) o 405, esperamos más tiempo.
                const delay = (error === 428 || error === 405) ? 10000 : 5000;
                console.log(`[WA] Reintentando en ${delay}ms...`);
                
                setTimeout(() => connectToWhatsApp(userId, phoneNumber), delay);
            } else {
                console.log(`[WA] Cierre definitivo (Logout). Limpiando sesión.`);
                sseService.sendEvent(userId, 'status_update', { status: ConnectionStatus.DISCONNECTED });
                await disconnectWhatsApp(userId);
            }
      ...
      console.error("[WA] Fatal Error creating socket:", err);
      sseService.sendEvent(userId, 'status_update', { status: ConnectionStatus.DISCONNECTED });
  }
}
