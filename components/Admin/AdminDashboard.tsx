        ...
        <div className="flex-1 bg-brand-black p-8 overflow-y-auto custom-scrollbar">
            <div className="max-w-[1400px] mx-auto space-y-8">
                <header className="flex justify-between items-end border-b border-white/10 pb-6">
                    <div>
                        <h2 className="text-2xl font-black text-white uppercase tracking-tighter">Global Control Node</h2>
                        <p className="text-[10px] text-gray-500 font-bold uppercase tracking-[0.2em] mt-1">SaaS Governance v2.6</p>
                    </div>
                    <div className="flex gap-4">
                        <span className="text-[10px] text-green-500 font-black px-3 py-1 bg-green-500/10 border border-green-500/20 rounded">HEALTHY</span>
                    </div>
                </header>

                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    {kpi("Vendors Totales", telemetry?.totalVendors || 0)}
                    {kpi("Nodos Online", telemetry?.activeNodes || 0, "text-green-500")}
                    {kpi("Signals (IA)", telemetry?.totalSignalsProcessed || 0, "text-brand-gold")}
                    {kpi("Risk Warning", telemetry?.riskAccounts || 0, telemetry?.riskAccounts ? "text-red-500" : "text-gray-500")}
                </div>
                                ...
                                <tr key={acc.id} className="hover:bg-white/5 transition-colors">
                                    ...
                                    <td className="p-6">
                                        <span className={`px-2 py-1 rounded text-[9px] font-black uppercase ${acc.settings.isActive ? 'bg-green-500/10 text-green-500' : 'bg-gray-500/10 text-gray-500'}`}>
                                            {acc.settings.isActive ? 'Online' : 'Paused'}
                                        </span>
                                    </td>
                                    <td className="p-6">
                                    ...
                                    <td className="p-6 text-right">
                                        <button onClick={() => onAudit(acc)} className="text-[9px] font-black uppercase px-4 py-2 bg-white/5 text-gray-400 border border-white/10 rounded hover:bg-white/10 hover:text-white transition-all">Audit Mode</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
